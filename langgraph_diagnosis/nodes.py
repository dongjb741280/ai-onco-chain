...
- 「治疗评价」表每行都要有「推荐等级 + 证据类别」列（如「Ⅰ级 / 1A类」）：有指南依据的行必须从【指南片段】填推荐等级与证据类别，不得留空；仅当该行无任何指南依据时才写「—」
- 「病理与分子分型依据」「治疗评价」「后续建议」中，凡依据指南的知识点，都要标注知识库来源，格式如「[指南名 · 章节 · P页码]」，来源取自【指南片段】里的【来源N】标注（指南+章节+页码）
- 指南引用优先级：以 {_PRIMARY_GUIDE} 为基础，按 {_GUIDE_PRIORITY} 顺序依次罗列各指南依据；某指南未覆盖该知识点则跳过，多条指南都覆盖时都列出
- 指南之间如有差异，不判定优劣，仅按上述顺序并列罗列各自来源
